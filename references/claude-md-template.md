...
`AGENTS.local.md` is a file that lives at the root of your vault.
...
When a user asks Claude to create their `AGENTS.local.md`, Claude should:
...
6. Write the file to `AGENTS.local.md` at the vault root (Write tool, or `obsidian_save_note` if using the bundled MCP server)

---

## The Template

Copy this, fill in the bracketed values, and save as `AGENTS.local.md` in the vault root.
...
This is a general operating principle, not vault-specific. Keep it in `AGENTS.local.md` so every Claude session in this vault inherits it.
...
*Regenerate with: "Claude, update my AGENTS.local.md"*
```

---

## Keeping `AGENTS.local.md` Fresh

`AGENTS.local.md` should be regenerated or updated when:
...
The user can trigger this with: *"Update my `AGENTS.local.md`"* or *"Regenerate my vault manifest."*
