...
Operating instructions for AI agents when working inside this repo.

This is the source repo for **obsidian-second-brain**, a cross-CLI AI agent skill that turns any Obsidian vault into a living AI-first second brain. The skill ships 44 slash commands across 4 layers (vault management, thinking tools, research toolkit, scheduled agents).

If you are an AI agent operating on a user's vault, read `_AGENTS.md` inside their vault first when present, with `_CLAUDE.md` as the legacy fallback. This file is for working on the skill's source code.
...
- `SKILL.md` - full operating manual loaded by Claude Code when the skill activates.
...
Source files in `commands/` use Claude Code's slash-command shape because that was the original host format. The Claude Code adapter is an identity copy. The Codex CLI, Gemini CLI, and OpenCode adapters emit a platform-appropriate dispatcher file (`AGENTS.md` or `GEMINI.md`) at the dist root with an **auto-generated routing table** built from each command's `description:` frontmatter, plus command bodies under `.codex/commands/`, `.gemini/commands/`, or `.opencode/commands/`. Tool-name references like `Read tool` are rewritten to neutral wording (`read files`) so the instructions still make sense outside Claude Code.
...
Every command that writes to a user's vault MUST follow `references/ai-first-rules.md`. Vault notes are designed for future AI-agent retrieval, not human reading. This means:

- A `## For future Claude` preamble in Claude Code builds, translated to `## For future AI agent` in non-Claude builds
...
- **Commits:** imperative mood (`Add`, `Fix`, `Update`). Co-author the assisting AI agent when pair-programmed.
...
3. **Apply the AI-first rule to every vault write.** Reference `references/ai-first-rules.md` from the command body so future AI agents have the spec inline.
...
Then restart the target CLI and run the command against a test vault. There is automated smoke coverage for the adapter build and vault health checker, but command-level vault verification is still manual: run the command, inspect the resulting vault notes, confirm AI-first compliance.
...
- Do not rewrite vault output to be "more human-friendly." The vault is for future AI agents, not human readers.
- Do not strip frontmatter or AI-first preambles from existing commands.
...
- **Maintainer (Eugeniu) and AI agents assisting him:** may push to `main` directly when working solo. PRs are optional, not mandatory.
